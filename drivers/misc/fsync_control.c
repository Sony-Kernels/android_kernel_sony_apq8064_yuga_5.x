...
static ssize_t fsynccontrol_status_read(struct device * dev,
...
static ssize_t fsynccontrol_status_write(struct device * dev,
					struct device_attribute * attr,
					const char * buf,
					size_t size)
{
...
static ssize_t fsynccontrol_version(struct device * dev,
					struct device_attribute * attr,
					char * buf)
{
...
static DEVICE_ATTR(fsync_enabled, S_IRUGO | S_IWUGO,
						fsynccontrol_status_read,
						fsynccontrol_status_write);
static DEVICE_ATTR(version, S_IRUGO , synccontrol_version, NULL);
...
static int __init fsynccontrol_init(void)
{
	...
	pr_info("%s misc_register(%s)\n", __func__, fsynccontrol_device.name);
